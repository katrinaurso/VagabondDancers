<?php # Admin - expired_events.php
// This page is for a logged-in administrator to be able to view all expired (last activy older than 90 days) entries into the events table

require ('includes/config.inc.php');
$page_title = 'View Expired Events';
...
// Page header:
echo '<h1>Expired Dance Events</h1>';
...
if (isset($_GET['np'])) { // Already been determined.
	...
	// Count the number of records (Please change the number from 10 to 90)
	$q = "SELECT COUNT(*) FROM events WHERE event_last_activity <= DATE_SUB(CURDATE(), INTERVAL 10 DAY)";
...
$link4 = "{$_SERVER['PHP_SELF']}?sort=daa";
...
if (isset($_GET['sort'])) {
	...
	switch ($_GET['sort']) {
		...
		case 'daa':
			...
			$link4 = "{$_SERVER['PHP_SELF']}?sort=dad";
			break;
		case 'dad':
...
WHERE event_last_activity <= DATE_SUB(CURDATE(), INTERVAL 10 DAY)
...
<tr>
	...
	<td align="left"><b><a href="' . $link4 . '">Last Activity</a></b></td>
</tr>';
